...
export const getSepoliaNetworkObject = (): NetworkProperties => {
  const { sepolia_enabled, sepolia_tx_enabled } = getRemoteConfig();
  return {
    // wagmi chain data
    ...sepolia,

    // network related data
    enabled: sepolia_enabled,
    name: 'Sepolia',
    longName: 'Sepolia',
    ...
    getProvider: getProviderForNetwork(Network.sepolia),
    rpc: proxyRpcEndpoint(sepolia.id),
